...
- Replaced the JSON-backed migration node implementation with package-owned internal save data nodes and a package-owned internal node factory.
- Kept `ISaveDataNode` and `ISaveDataNodeFactory` as the public migration edit surface while removing Newtonsoft `JToken` from the migration node backing model.
- Updated `JsonSaveSerializer` migration support to convert UTF-8 JSON bytes to package-owned nodes before migration and convert nodes back to formatted JSON afterward.
- Preserved serializer/factory ownership checks so nodes from different factories or serializer instances cannot be mixed.
- Updated data-node tests, public API shape coverage, README guidance, and XML documentation for the format-neutral node model.
- `dotnet test Workes.SaveSystem.sln` passes with 179 tests.
...
### 63. Converted Serializer Payloads To Bytes

- Changed serializer and schematic contracts from string payloads to `byte[]` payloads.
- Updated JSON serialization to read and write UTF-8 JSON bytes while keeping `.json` files human-readable.
- Updated manager persistence, metadata reads/writes, temp-save validation, recovery validation, schema-version extraction, and migration plumbing to pass byte payloads.
- Updated test serializers and serializer contract coverage for the byte-oriented API.
- Updated README and XML documentation for the byte-oriented serializer contract.

### 64. Added JSON Formatting Options

- Added `JsonSaveFormatting` with `Pretty` and `Compact` options.
- Kept `new JsonSaveSerializer()` as pretty JSON by default.
- Added `new JsonSaveSerializer(JsonSaveFormatting.Compact)` for compact `.json` output.
- Preserved JSON formatting choice through provider serialization and migration serialization.
- Updated README and serializer output examples to show pretty and compact JSON.
